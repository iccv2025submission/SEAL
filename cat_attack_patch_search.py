    ...
    for k in args.k_values:
        for b in args.b_values:
            ...
            for img_idx in tqdm(range(args.start, args.end), desc=f'k={k}, b={b}'):
                ...
                random_noise = generate_noise_from_reconstructed(
                    recon_noise_random, k, b, 42, device, refinement_steps=1
                ...
                org_noise = generate_noise_from_reconstructed(
                    recon_noise_org, k, b, 42, device, refinement_steps=1
                ).to(dtype=pipe.vae.dtype)

                # Generate the noise for the cat image using enhanced brute-force search
                cat_noise = generate_noise_from_reconstructed(
                    recon_noise, k, b, 42, device, refinement_steps=1
                ).to(dtype=pipe.vae.dtype)
